// Package reso contains an out-of-tree plugin based on the Kubernetes
...
import (
	...
	"github.com/dimitrisdol/resoScheduler/reso/hardcoded"
...
type ResoPlugin struct {
	handle framework.Handle
	model  InterferenceModel
}

var (
	_ framework.Plugin          = &ResoPlugin{}
	_ framework.FilterPlugin    = &ResoPlugin{}
	_ framework.ScorePlugin     = &ResoPlugin{}
	_ framework.ScoreExtensions = &ResoPlugin{}
)

// New instantiates a ResoPlugin.
func New(configuration runtime.Object, f framework.Handle) (framework.Plugin, error) {
	return &ResoPlugin{
		handle: f,
		model:  hardcoded.New(resoLabelKey),
	}, nil
}

// Name returns the official name of the ResoPlugin.
func (_ *ResoPlugin) Name() string {
	return Name
}

// findCurrentOccupants returns all Pods that are being tracked by ResoPlugin
...
func (_ *ResoPlugin) findCurrentOccupants(nodeInfo *framework.NodeInfo) []*corev1.Pod {
...
func (ap *ResoPlugin) Filter(
	...
	// If the given Pod does not have the resoLabelKey, approve it and let
	// the other plugins decide for its fate.
	if _, exists := pod.Labels[resoLabelKey]; !exists {
		klog.V(2).Infof("blindly approving Pod '%s/%s' as it does not have ResoPlugin's label %q", pod.Namespace, pod.Name, resoLabelKey)
		return framework.NewStatus(framework.Success, "pod is not tracked by ResoPlugin")
	}

	// For the Node at hand, find all occupant Pods tracked by ResoPlugin.
	...
	// If the Node is full (i.e., 2 applications tracked by ResoPlugin are
	// already scheduled on it), filter it out.
	case 2:
		klog.V(2).Infof("filtering Node %q out because 2 ResoPlugin applications are already scheduled there", nodeName)
		return framework.NewStatus(framework.Unschedulable, fmt.Sprintf("Node '%s' already has 2 ResoPlugin occupants", nodeName))
	...
	case 1:
		...
		if score > sla {
			msg := fmt.Sprintf("filtering Node '%s': new pod '%s/%s' ('%s') incurs huge slowdown on pod '%s/%s' ('%s')",
				nodeName, pod.Namespace, pod.Name, pod.Labels[resoLabelKey], occ.Namespace, occ.Name, occ.Labels[resoLabelKey])
			klog.V(2).Infof(msg)
	...
	default:
		klog.Errorf("detected %d occupant Pods tracked by ResoPlugin on Node %q", len(occupants), nodeName)
...
//
// In the case of ResoPlugin, scoring is reversed; i.e., higher score indicates
// worse scheduling decision.
// This is taken into account and "fixed" later, during the normalization.
func (ap *ResoPlugin) Score(
	ctx context.Context,
	state *framework.CycleState,
	p *corev1.Pod,
	nodeName string,
...
// ScoreExtensions returns the ResoPlugin itself, since it implements the
// framework.ScoreExtensions interface.
func (ap *ResoPlugin) ScoreExtensions() framework.ScoreExtensions {
	return ap
}
...
//
// In the case of the ResoPlugin, its "Score" method produces scores of reverse
...
func (_ *ResoPlugin) NormalizeScore(
	ctx context.Context,
	state *framework.CycleState,
	p *corev1.Pod,
	...
	// When no Pod (tracked by ResoPlugin) is scheduled on the Node,
	// maxScore will be 0.
	if maxScore == 0 {
		for i := range scores {
