...
// HardcodedSlowDowns is an implementation of reso.InterferenceModel, where the
...
// is used by ResoPlugin to track its applications).
func New(resoLabelKey string) *HardcodedSlowDowns {
	return &HardcodedSlowDowns{
		resoLabelKey: resoLabelKey,
	}
}
...
func (m *HardcodedSlowDowns) Attack(attacker, occupant *corev1.Pod) (float64, error) {
	...
	newPodCategory, err := parseAppCategory(attacker.Labels[m.resoLabelKey])
	if err != nil {
		return -1, err
	}
...
// ToInt64Multiplier implements reso.InterferenceModel; see the documentation
...
// the 4 appCategories that are to be examined by the ResoPlugin are as
// follows:
...
var slowDowns = slowDownMatrix{
	catA: map[appCategory]float64{
		catA: 1.48,
		catB: 1.05, // slowdown of catB when attacked by catA = 1.05
		catC: 1.45,
		catD: 1.01,
	},
	...
	catC: map[appCategory]float64{
		catA: 1.38,
		catB: 2.35,
		catC: 1.02, // slowdown of catC when attacked by catC = 1.02
		catD: 3.57,
	},
